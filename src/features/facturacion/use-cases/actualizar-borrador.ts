...
import { actualizarCabecera, esBorradorDelCliente, reemplazarConceptos } from "../repositories/facturas-repository";
import type { CrearBorradorData, FacturaMutationResult } from "../types";

export async function actualizarBorradorUseCase(
  idFactura: number,
  idCliente: number,
  data: CrearBorradorData,
): Promise<FacturaMutationResult> {
  const esBorrador = await esBorradorDelCliente(idFactura, idCliente);
  if (!esBorrador) return { ok: false, error: "Esta factura ya no se puede editar." };
  ...
  await actualizarCabecera(idFactura, data);
  await reemplazarConceptos(idFactura, data.conceptos);
